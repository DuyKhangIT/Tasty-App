...
public class LocationAdapter extends RecyclerView.Adapter<LocationAdapter.ViewHoler> {

    FirstFragment context;
    int layout;
    ArrayList<Location> arrayList;

    public LocationAdapter(FirstFragment context, int layout, ArrayList<Location> arrayList) {
    ...
    @NonNull
    @Override
    public LocationAdapter.ViewHoler onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context.getActivity()).inflate(layout, parent, false);
        return new ViewHoler(v);
    }

    @Override
    public void onBindViewHolder(@NonNull LocationAdapter.ViewHoler holder, int position) {
    ...
    private void onClickGoToDetail(Location listData) {
        Intent intent = new Intent(context.getActivity(), DetailActivity.class);
